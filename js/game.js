function Game() {
  ...
  this.play = function () {
    ...
    if (
      this.bird.top >
      this.containerHeight - this.groundHeight - this.bird.HEIGHT
    ) {
      this.bird.angle = 90;
      this.bird.setAngle();
      this.gamePaused = true;
    ...
    if (!this.gamePaused) {
      ...
      for (var i = 0; i < this.upperPipes.length; i++) {
        ...
        if (
          this.upperPipes[i].left < this.bird.left - this.bird.WIDTH / 2 &&
          !this.upperPipes[i].hasPassed
        ) {
  ...
  this.checkCollisionPipes = function (i) {
    if (
      (this.bird.left < this.upperPipes[i].left + this.upperPipes[i].width &&
  ...
  this.addPipes = function () {
    ...
    if (this.pipeCounter == 0) {
      ...
      var lowerPipeTop =
        this.containerHeight - this.groundHeight - lowerPipeHeight;
  ...
  this.addBackgroundWrapper = function () {
    ...
    this.backgroundWrapper.style.height =
      this.containerHeight - this.groundHeight + "px";

    this.container.appendChild(this.backgroundWrapper);

    var imgSrc = ["images/bg.png"];

    this.backgroundWrapper.appendChild(
      this.getImage(
        this.containerWidth,
        this.backgroundWrapper.style.height,
        imgSrc[0]
  ...
  this.addForegroundWrapper = function () {
    ...
    var imgSrc = ["images/foreground.png"];

    this.foregroundWrapper.appendChild(
      this.getImage(
        this.containerWidth,
        this.foregroundWrapper.style.height,
        imgSrc[0]
      )
    );
